...
<body class="flex flex-col h-full bg-gray-900 bg-slate-900 text-gray-300">
    ...
    if (isset($_POST['username']) && isset($_POST['password'])) {
        function error(string $error): void {
            ...
            switch ($error) {
                ...
                case "userExists":
                    $message = "<p class='block text-gray-300 dark:text-gray-300 text-sm font-bold mb-2'>" . "<span class='text-red-800'>Error: </span>" . 'Der Benutzer existiert nicht/Existiert bereits in dieser Form!' . "</p>";
                    break;
                ...
                default:
                    $message = "<p class='block text-gray-300 dark:text-gray-300 text-sm font-bold mb-2'>" . 'User wurde erfolgreich geändert' . "</p>";
                    break;
            }
            ...
            if ($row) {
                try {
                    if (preg_match('@[A-Z]@', $pw) && preg_match('@[a-z]@', $pw) && preg_match('@[0-9]@', $pw) && preg_match('@\w@', $pw) && strlen($pw) > 10) {
                        $stmt = $conn->prepare("UPDATE login.users SET pw = SHA1(:pw) WHERE username = :username");
                        $stmt->bindParam(':username', $username);
    ...
    <form action="<?php $_SERVER['PHP_SELF'] ?>" class="w-full max-w-md mx-auto bg-gray-800 shadow-md rounded px-8 py-6 mt-8" method="post">
        <h1 class="text-xl font-black text-gray-100">Reset your password</h1>
        <div class="my-4 mt-8">
        ...
        <button class="button mt-4 w-full bg-indigo-500 py-2.5 rounded" type="submit">Reset →</button>
    </form>
